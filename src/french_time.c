...
static char* const STR_DIX_H = "dix";
static char* const STR_VINGT_H = "vingt";
static const char* STR_H = " h.";

static char* SEIZEHEURES[] = {
	"minuit","une","deux","trois","quatre","cinq","six","sept","huit","neuf","dix","onze","midi","treize","quatorze","quinze","seize"};
static char* DIXHEURES[] = {"sept","huit","neuf","vingt"};
static char* VINGTHEURES[] = {"une","deux","trois"};
...
int textwatch_time(char* line1, char* line2, char* line3, char* line4, struct tm * t) {
  ...
  if (hours >= 1 && hours <= 11) //On affiche Heure sur la seconde ligne
  {
    strcat(line1, SEIZEHEURES[hours]); //on affiche l'heure (toujours en première ligne)
  ...
  if (hours == 0 || hours == 12)
  {
		strcat(line1, SEIZEHEURES[hours]);    
		strcat(line2, mina);
    strcat(line3, minb);
  }
  if (hours >= 13 && hours <= 16) //On affiche Heure sur la seconde ligne
  {
    strcat(line1, SEIZEHEURES[hours]); //on affiche l'heure (toujours en première ligne)
    strcat(line2, STR_HEURE);
    strcat(line3, mina);
 ...
 if (hours >= 17 && hours <= 19) //On affiche "h." sur la seconde ligne
  {
    strcat(line1, STR_DIX_H);
		strcat(line2, DIXHEURES[hours-17]); //on affiche l'heure (toujours en première ligne)
    strcat(line2, STR_H);
    strcat(line3, mina);
    strcat(line4, minb);
  }

if (hours == 20) //On affiche "heures" sur la seconde ligne
  {
		strcat(line1, DIXHEURES[hours-17]); //on affiche l'heure (toujours en première ligne)
    strcat(line2, STR_HEURE);
    strcat(line3, mina);
    strcat(line4, minb);
	  if (hours > 1) strcat(line2, "s"); //rajoute un "s" à heure
...
if (hours >= 21 && hours <= 23) //On affiche "h." sur la seconde ligne
  {
    strcat(line1, STR_VINGT_H);
		strcat(line2, VINGTHEURES[hours-21]); //on affiche l'heure (toujours en première ligne)
		strcat(line2, STR_H);
    strcat(line3, mina);
    strcat(line4, minb);
	}
